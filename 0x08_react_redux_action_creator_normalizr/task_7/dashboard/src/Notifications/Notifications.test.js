...
describe("Testing the <Notifications /> Component", () => {
	...
	it("<Notifications /> renders the first <NotificationItem /> element with the right HTML", () => {
		...
		wrapper = shallow(<Notifications displayDrawer={true} listNotifications={listNotifications}/>);
		expect(wrapper.find('ul').childAt(0).html()).to.equal('<li data-priority=\"default\" class=\"Default_876n4g\">New course available</li>');
	});
	
	it("<Notifications /> the menu item is being displayed when displayDrawer is false", () => {
		
		expect(wrapper.find('div.menuItem_11rhnke-o_O-hide_1u9fru1')).to.have.length(1);
	});
	...
	it("<Notifications /> the div.Notifications is not being displayed when displayDrawer is false", () => {
		...
		expect(wrapper.find('div.Notifications_1mv3ktq')).to.have.length(1);
	});
...
describe('updating the props of the component', () => {
	/* it('with the same list, the component doesn’t rerender', () => {
		...
		it("Verify that clicking on the menu item calls handleDisplayDrawer", () => {
			...
			wrapper.find('.menuItem_11rhnke p').at(0).simulate('click');
		...
		it("Verify that clicking on the button calls handleHideDrawer", () => {
			...
			wrapper.find('.buttonStyle_86u0bw').at(0).simulate('click');
